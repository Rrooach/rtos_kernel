...
#define  NI_MAXSERV                                32u
                                                                ...
                                                                /* ------------------ FILE DESC SETS ------------------ */
#define  _FD_SETSIZE                NET_SOCK_NBR_SOCK            /* See Note #5a2A in "BSD 4.x SOCKET DATA TYPES" sect.  */
#define  FD_MIN                                 0               /* See Note #5a2B in "BSD 4.x SOCKET DATA TYPES" sect.  */
#define  FD_MAX                   (_FD_SETSIZE - 1)
#define  FD_ARRAY_SIZE          (((_FD_SETSIZE - 1) / (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS)) + 1)
...
*
*                   (2) (A) "_suseconds_t - Used for time in microseconds."
*
*                       (B) "_suseconds_t shall be a signed integer type capable of storing values at least
*                            in the range [-1, 1000000]."
*
*               (c) "The implementation shall support ... size_t, ssize_t, _suseconds_t ... widths ... no
*                    greater than the width of type long."
*
*           (4) (a) IEEE Std 1003.1, 2004 Edition, Section 'sys/select.h : DESCRIPTION' states that "the
*                   '_timeval' structure ... includes at least the following members" :
*
*                   (1) time_t         tv_sec     Seconds
*                   (2) _suseconds_t    tv_usec    Microseconds
*
*               (b) Ideally, the BSD 4.x Layer's '_timeval' data type would be the basis for the Network
...
*
*                   However, for correct interoperability between the BSD 4.x Layer '_timeval' data type
...
*                       the "'_fd_set' type ... shall [be] define[d] ... as a structure".
*
*                       (A) "The requirement for the '_fd_set' structure to have a member 'fds_bits' has
*                            been removed."
*
...
*                           that "_FD_SETSIZE ... shall be defined as a macro ... [as the] maximum number
*                           of file descriptors in an '_fd_set' structure."
*
*                       (B) Stevens/Fenner/Rudoff, UNIX Network Programming, Volume 1, 3rd Edition,
*                           6th Printing, Section 6.3, Page 163 states that "descriptors start at 0".
*
*               (b) Ideally, the BSD 4.x Layer's '_fd_set' data type would be the basis for the Network
...
*                   data type MUST be independently defined.
*
*                   However, for correct interoperability between the BSD 4.x Layer '_fd_set' data type
...
typedef  CPU_INT32S       _suseconds_t;                                  /* Signed time val in usec (see Note #3b2).     */



struct  _timeval {                                                       /* See Note #4a.                                */
   _time_t                tv_sec;                                       /* Time val in  sec (see Note #4a1).            */
    _suseconds_t           tv_usec;                                      /* Time val in usec (see Note #4a2).            */
};


struct  _fd_set {                                                        /* See Note #5a1.                               */
...
*                               "file descriptor masks of type '_fd_set' can be initialized and tested with" :
*
*                               (1) "_FD_ZERO(fdsetp) shall initialize the descriptor set pointed to by 'fdsetp'
*                                    to the null set.  No error is returned if the set is not empty at the time
*                                    _FD_ZERO() is invoked."
*
*                               (2) "_FD_CLR(fd, fdsetp) shall remove the file descriptor 'fd' from the set pointed
*                                    to by 'fdsetp'.  If 'fd' is not a member of this set, there shall be no effect
*                                    on the set, nor will an error be returned."
*
*                               (3) "_FD_SET(fd, fdsetp) shall add the file descriptor 'fd' to the set pointed to by
*                                    'fdsetp'.  If the file descriptor 'fd' is already in this set, there shall be
*                                    no effect on the set, nor will an error be returned."
*
*                               (4) "_FD_ISSET(fd, fdsetp) shall evaluate to non-zero if the file descriptor 'fd' is a
...
*                                   (a) "less than 0"                             ...
*                                   (b) "or greater than or equal to _FD_SETSIZE," ...
*
...
#define  _FD_ZERO(fdsetp)    do {                                                                              \
                                if (((struct _fd_set *)(fdsetp)) != (struct _fd_set *)0) {                      \
                                    Mem_Clr ((void     *)      (&(((struct _fd_set *)(fdsetp))->fds_bits[0])), \
                                             (CPU_SIZE_T) (sizeof(((struct _fd_set *)(fdsetp))->fds_bits)));   \
                                }                                                                             \
                            } while (0)


#define  _FD_CLR(fd, fdsetp)             do {                                                                                                              \
                                            if (((fd) >= FD_MIN) && ((fd) <= FD_MAX) &&                                                                   \
                                                                 (((struct _fd_set *)(fdsetp)) != (struct _fd_set *)0)) {                                   \
                                                  DEF_BIT_CLR   ((((struct _fd_set *)(fdsetp))->fds_bits[(fd) / (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS)]), \
                                                  DEF_BIT                                              ((fd) % (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS))); \
                                            }                                                                                                             \
                                        } while (0)

#define  _FD_SET(fd, fdsetp)             do {                                                                                                              \
                                            if (((fd) >= FD_MIN) && ((fd) <= FD_MAX) &&                                                                   \
                                                                 (((struct _fd_set *)(fdsetp)) != (struct _fd_set *)0)) {                                   \
                                                  DEF_BIT_SET   ((((struct _fd_set *)(fdsetp))->fds_bits[(fd) / (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS)]), \
                                                  DEF_BIT                                              ((fd) % (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS))); \
                                            }                                                                                                             \
                                        } while (0)

#define  _FD_ISSET(fd, fdsetp)                  ((((fd) >= FD_MIN) && ((fd) <= FD_MAX) &&                                                                  \
                                                                 (((struct _fd_set *)(fdsetp)) != (struct _fd_set *)0)) ?                                   \
                                               (((DEF_BIT_IS_SET((((struct _fd_set *)(fdsetp))->fds_bits[(fd) / (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS)]), \
                                                  DEF_BIT                                              ((fd) % (sizeof(CPU_DATA) * DEF_OCTET_NBR_BITS)))) \
                                               == DEF_YES) ? 1 : 0)                                                                                       \
                                                               : 0)
       ...
       ssize_t       send        (               int         sock_id,
                                                ...
                                                _size_t      data_len,
                                                 int         flags);
                                                                        ...
                                                                        /* ------------ MULTIPLEX I/O FNCTS ----------- */
       int           _select      (               int         desc_nbr_max,
                                         struct  _fd_set     *p_desc_rd,
                                         struct  _fd_set     *p_desc_wr,
                                         struct  _fd_set     *p_desc_err,
                                         struct  _timeval    *p_timeout);
