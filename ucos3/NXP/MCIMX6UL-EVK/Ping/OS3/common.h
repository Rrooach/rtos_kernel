...
#define htobe64 _byteswap___uint64_t
#define le16toh(x) x
#define htole16(x) x
...
static void sleep_ms(__uint64_t ms)
{ 
	// usleep(ms * 1000);
}
#endif
...
// #if GOOS_freebsd || GOOS_darwin || GOOS_netbsd || GOOS_openbsd || GOOS_akaros || GOOS_test || GOOS_UCOS
#if SYZ_EXECUTOR || SYZ_THREADED
...
static void csum_inet_update(struct csum_inet* csum, const __uint8_t* data, size_t length)
{
	if (length == 0)
		return;
	...
	size_t i = 0;
	for (; i < length - 1; i += 2)
		csum->acc += *(__uint16_t*)&data[i];

	if (length & 1)
		csum->acc += (__uint16_t)data[length - 1];
		// csum->acc += le16toh((__uint16_t)data[length - 1]);

	while (csum->acc > 0xffff)
		csum->acc = (csum->acc & 0xffff) + (csum->acc >> 16);
}

static __uint16_t csum_inet_digest(struct csum_inet* csum)
{
	return ~csum->acc;
}
#endif
...
static void loop(void)
{
	...
	for (;; iter++) {
		...
		int status = 0;
		__uint64_t start = current_time_ms();
#if SYZ_EXECUTOR && SYZ_EXECUTOR_USES_SHMEM
		__uint64_t last_executed = start;
		__uint32_t executed_calls = __atomic_load_n(output_data, __ATOMIC_RELAXED);
#endif
		for (;;) {
			...
			__uint64_t min_timeout_ms = program_timeout_ms * 3 / 5;
			__uint64_t inactive_timeout_ms = syscall_timeout_ms * 20;
			__uint64_t now = current_time_ms();
			__uint32_t now_executed = __atomic_load_n(output_data, __ATOMIC_RELAXED);
